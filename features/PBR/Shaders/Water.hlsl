...
cbuffer PerTechnique : register(b0)
{
	...
	float4 SunColor2 : packoffset(c4);
}
...
#		else
	if (shaderDescriptors[0].PixelShaderDescriptor & _Interior)
		return 1;
	return saturate(dot(SunDirection.xyz, normal));
#		endif
}
...
#		if defined(REFRACTIONS)
	...
	float distanceMul2 = distanceMul.z;
	distanceMul2 = max(distanceMul2, distanceFactor);
	float distanceMul3 = distanceMul.x;

#			if defined(DEPTH)
	float depth = distanceMul.y;
	float refractionDepth =
		GetScreenDepth(DynamicResolutionParams1.xy * (saturate(refractionUvRaw) / VPOSOffset.xy));
	float refractionDepthMul = length(
...
#			endif
	...
	float MaxShallowColor = max(ShallowColor.x, max(ShallowColor.y, ShallowColor.z));
	float AttenuationColorScale = WaterScatter / MaxShallowColor;
	AttenuationColorScale = max(AttenuationColorScale, 1);

	float2 refractionUV = GetDynamicResolutionAdjustedScreenPosition(refractionUvRaw);
	float3 refractionColor = RefractionTex.Sample(RefractionSampler, refractionUV).xyz;

	float3 refractionDiffuseColor = lerp(ShallowColor.xyz, DeepColor.xyz, saturate(distanceMul4));
	//矫正亮度
	refractionDiffuseColor *= AttenuationColorScale;

	//float3 FinalColor = refractionColor * exp((refractionDiffuseColor.xyz - 1) * distanceMul3 * 2);
	float3 FinalColor = refractionColor * refractionDiffuseColor;

	float3 LTLight = SunDirection.xyz + normal;
	float LTDot = saturate(dot(viewDirection, LTLight));
	LTDot = pow(LTDot, 12);
	FinalColor += LTDot * ShallowColor.xyz * 10;
	...
	//float AmbientScatter = 1 - normal.z * normal.z;
	//FinalColor += AmbientScatter * fresnel * 30 * DeepColor.xyz;
...
#			endif
	//return lerp(refractionColor * WaterParams.w, refractionDiffuseColor, refractionMul);
	return lerp(refractionColor * WaterParams.w, FinalColor, refractionMul);
	//return lerp(refractionDiffuseColor, refractionColor * WaterParams.w, exp(-distanceMul3 * 4));
#		else
	return lerp(ShallowColor.xyz, DeepColor.xyz, fresnel) * GetLdotN(normal);
#		endif
}
...
#		else
	...
	float reflectionMul = exp2(VarAmounts.x * log2(saturate(dot(reflectionDirection, SunDirection.xyz))));

	float3 sunDirection = SunColor.xyz;
...
float3 GetSunColor2(float3 normal, float3 viewDirection, float fresnel, float Roughness)
{
...
#		else
	...
	float Vis = Vis_SmithJointApprox_invert(NoV, NoL, a);
...
#	if defined(SIMPLE) || defined(UNDERWATER) || defined(LOD) || defined(SPECULAR)
	...
	float distanceFactor2 = saturate(input.WPosition.w * 0.1 / WaterParams.x);
	//float ScreenRoughness = WaterRoughness * (1 + distanceFactor2);
	float ScreenRoughness = WaterRoughness * (1 + distanceFactor2);
	float distanceFactor3 = distanceMul.x;
...
#		endif
	...
	float fresnel = FastfresnelSchlick(NoV, 0.02);
	//float fresnel = EnvBRDFApproxNonmetal(ScreenRoughness, NoV);
...
#		else
	...
	//specularColor *= WaterReflection;
	float3 diffuseColor =
		GetWaterDiffuseColor2(input, normal, viewDirection, distanceMul, depthControl.y, fresnel, distanceFactor3);
...
#			if defined(UNDERWATER)
	...
	float3 finalColor = saturate(1 - input.WPosition.w * 0.002) *
	                    ...
	                    finalSpecularColor;
#			else
	float3 sunColor = GetSunColor2(normal, viewDirection, fresnel, ScreenRoughness);
	float specularFraction = lerp(1, fresnel * saturate(depthControl.x), distanceFactor);
...
#		endif
	...
	psout.Lighting = float4(finalColor * PosAdjust.w, isSpecular);
	psout.Lighting.xyz = SampleAerialPerspectiveLut(input.WPosition.xyz, psout.Lighting.xyz, RefractionSampler);
...
#				else
		float blendFactor = 1 - smoothstep(0.0, 0.025 * perPassWaterBlending[0].WaterBlendRange, saturate(distanceMul.z));
